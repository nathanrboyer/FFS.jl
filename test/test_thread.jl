@testset "Thread" begin
    ...
    @test RCSCCL3.P_m(1, 2, 3, 4, 5) == 5
    @test RCSCCL3.P_b(123123, 2, 2, 20/9, 15/6) == -4
    @test isapprox(RCSCCL3.U(0.25, 2.5), 1.269625, atol = 1e-4)
    @test isapprox(RCSCCL3.Σ_m(0, 0.48708932, 8.2995), 4.042598, atol = 1e-4)
    ...
    @test isapprox(RCSCCE1.σ_mc(60, 3.613, 6.317), 34.3169226, atol = 1e-4)
    @test isapprox(RCSCCE1.σ_ml(
        0, 3.613, 9.93, 1698.157, 8.2995), 14.61783, atol = 1e-4)
    @test isapprox(RCSCCE1.P_meq(34.3169226, 14.61783), 29.827725, atol = 1e-4)
    @test isapprox(RCSCCE1.σ_ref(
